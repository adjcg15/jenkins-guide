    ...
    stage('Deploy') {
      ...
      steps {
        script {
          ...
          sshCommand (remote: remote, command:"cd /d C:\\deploy\\todolist && curl -L -o docker-compose.yml https://raw.githubusercontent.com/adjcg15/jenkins-guide/deployment-pipeline/XXXX/docker-compose.yml && docker compose up -d && docker compose ps")
            sleep 15 
        }
